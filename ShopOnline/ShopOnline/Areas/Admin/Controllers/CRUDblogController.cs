...
namespace ShopOnline.Areas.Admin.Controllers
{
    ...
    public class CRUDblogController : Controller
    {
        ...
        public ActionResult Create(Article article, HttpPostedFileBase uploadFile)
        {
            try
            {
                ...
                if (extension.ToLower() == ".jpg" || extension.ToLower() == ".jpeg" || extension.ToLower() == ".png")
                {
                    if (uploadFile == null)
                    {
                        ModelState.AddModelError("", "Lỗi khi tải tập tin lên");
                    }
                    else
                    {
                        ...
                        if (db.SaveChanges() > 0)
                        {
                            ...
                            ModelState.Clear();
                            TempData["msgCreate"] = "Tạo thành công một blog mới!";
                ...
                {
                    ModelState.AddModelError("", "Loại tệp không hợp lệ");
                }
            ...
            {
                TempData["msgCreatefailed"] = "Tạo không thành công! " + ex.Message;
        ...
        public ActionResult Edit(Article article, HttpPostedFileBase uploadFile, FormCollection collection)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (uploadFile != null)
                    {
                        ...
                        if (db.SaveChanges() > 0)
                        {
                            TempData["msgEdit"] = "Sản phẩm đã chỉnh sửa thành công có ID: " + article.articleId;
                            uploadFile.SaveAs(path);
            ...
            {
                TempData["msgEditFailed"] = "Chỉnh sửa không thành công! " + ex.Message;
                return RedirectToAction("Index");
            }
        }
        //DELETE
        public ActionResult Delete(int? id)
        {
            ...
            {
                TempData["msgDelete"] = "Không thể xóa! " + ex.Message;
                return RedirectToAction("Index");
            }
        }
